...
class Help(commands.Cog):
    ...
    async def help(self, ctx, section=None):
        if section is None:
            ...
            embed = discord.Embed(title=":page_facing_up: Inventory Commands",
                                  description="**Note:** Items in these commands don't contain spaces",
                                  color=discord.Color.purple())
            embed.set_footer(text="Inventory command list")
            embed.add_field(name=f"`{self.bot.prefix}inv [page]/[user] [page]`", value="Open your inventory",
                            inline=False)
            ...
            embed.add_field(name=f"`{self.bot.prefix}lock (item)`",
                            value="Prevent an item from being traded or stolen, requires :lock: Lock", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}unlock (item)`",
                            value="Allow an item to be traded or stolen, requires :key: Key", inline=False)
            return await ctx.send(embed=embed)

        elif section.lower() == "economy":
            embed = discord.Embed(title=":credit_card: Economy Commands",
                                  description="**Note:** Items in these commands don't contain spaces",
                                  color=discord.Color.purple())
            embed.set_footer(text="Economy command list")
            embed.add_field(name=f"`{self.bot.prefix}balance [user]`", value="View yours or someone else's balance",
                            inline=False)
            ...
            embed.add_field(name=f"`{self.bot.prefix}sell`", value="Sell any item for 75% of its value to the shop",
                            inline=False)
            ...
            embed.add_field(name=f"`{self.bot.prefix}pshop buy (user) (item)`", value="Buys a user's item",
                            inline=False)
            embed.add_field(name=f"`{self.bot.prefix}pshop add (item) (price) [quantity]`",
                            value="Adds an item from your inventory to your shop. Default quantity : `1`", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}pshop remove (item) [quantity]`",
                            value="Removes a certain item from your shop. Default quantity : `1`", inline=False)
            ...
            embed.add_field(name=f"`{self.bot.prefix}withdraw (amount)`", value="Withdraw from your bank", inline=False)
            return await ctx.send(embed=embed)
            ...
            embed = discord.Embed(title=":moneybag: Crime Commands",
                                  description="**Note:** Items in these commands don't contain spaces",
                                  color=discord.Color.purple())
            embed.set_footer(text="Crime command list")
            embed.add_field(name=f"`{self.bot.prefix}rob (user) (tool) (desired item)`",
                            value="Rob another player of an item, requires a tool, leave blank to see owned tools",
                            inline=False)
            embed.add_field(name=f"`{self.bot.prefix}steal (user) (amount)`",
                            value="Rob another player of money, requires a :gun: Gun, min=500 max=7500", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}dynamite (user)`",
                            value="Blow up 20% of someone's balance, requires :firecracker: Dynamite", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}bomb (user)`",
                            value="Blow up 10% of someone's bank balance, requires :bomb: Bomb", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}burn (user) (item)`",
                            value="Burn another user's item, requires :fire: Fire", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}axe (user)`",
                            value="Unlock another player's item, requires :axe: Axe", inline=False)
            return await ctx.send(embed=embed)
            ...
            embed = discord.Embed(title=":scales: Trading Commands",
                                  description="**Note:** Items in these commands don't contain spaces",
                                  color=discord.Color.purple())
            embed.set_footer(text="Trading command list")
            embed.add_field(name=f"`{self.bot.prefix}trade (user) (owned item) (desired item)`",
                            value="Offer to trade with another player", inline=False)
            ...
            embed.add_field(name=f"`{self.bot.prefix}give (user) (item) [quantity]`",
                            value="Give an item to another player", inline=False)
            return await ctx.send(embed=embed)

        elif section.lower() == "research":
            embed = discord.Embed(title=":microscope: Research Commands",
                                  description="**Note:** Items in these commands don't contain spaces",
                                  color=discord.Color.purple())
            embed.set_footer(text="Research command list")
            embed.add_field(name=f"`{self.bot.prefix}iteminfo (item)`", value="Get information about an item",
                            inline=False)
            embed.add_field(name=f"`{self.bot.prefix}find (item)`",
                            value="Find 5 random users and their IDs who own an item", inline=False)
            return await ctx.send(embed=embed)
            ...
            embed.add_field(name=f"`{self.bot.prefix}feed`",
                            value="Feed a frog to a dragon, there's a 1% chance of something special happening",
                            inline=False)
            embed.add_field(name=f"`{self.bot.prefix}avatar [user]`", value="Display the avatar of a user in full size",
                            inline=False)
            embed.add_field(name=f"`{self.bot.prefix}8ball (question)`", value="Ask the magic 8ball a question!",
                            inline=False)
            embed.add_field(name=f"`{self.bot.prefix}cookie (user)`",
                            value="Sends a user a cookie. Purely cosmetic and yields no item", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}flower (user)`",
                            value="Sends a user a flower. Purely cosmetic and yields no item", inline=False)
            return await ctx.send(embed=embed)

        elif section.lower() == "leaderboards" or section.lower() == "leaders":
            embed = discord.Embed(title=":medal: Leaderboards",
                                  description="**Note:** Items in these commands don't contain spaces",
                                  color=discord.Color.purple())
            embed.set_footer(text="Leaderboards command list")
            embed.add_field(name=f"`{self.bot.prefix}leaderboard`",
                            value="Show the top 10 players with the highest total value", inline=False)
            ...
            embed.add_field(name=f"`{self.bot.prefix}diagnose`",
                            value="See which modules are online and check for errors", inline=False)
            embed.add_field(name=f"`{self.bot.prefix}ping`", value="Pong!", inline=False)
            return await ctx.send(embed=embed)

        data = await self.bot.inventories.find(ctx.author.id)
        if data is None:
            embed = discord.Embed(title=":herb: Lyfé Command List",
                                  description=f"**Hey!** Initialize your inventory with `{self.bot.prefix}inv`",
                                  color=discord.Color.purple())
        else:
            ...
            embed.add_field(name=":bust_in_silhouette: Profiles", value=f"`{self.bot.prefix}help profiles`",
                            inline=False)
